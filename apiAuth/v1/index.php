...
Flight::route('POST /authApiKey', function () {
    ...
    require_once '../../apiAuth/v1/controller/validate_api_key.php';

    $validate_api_key = new validate_api_key();
    $validation = $validate_api_key->validate_token($api_key);

    echo $validation;
});


Flight::route('POST /authCode', function () {
    $conectar = conn();
    //$uri = $_SERVER['REQUEST_URI'];
    ...
    $validate_api_code = new validate_api_key();
    $validation = $validate_api_code->validate_token($api_key);
